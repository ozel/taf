...
typedef union {
  ...
  struct
  {
    UInt16 Code    :  3; /*!< B00..B02 = Code, not used on MSis1, access via f.Code     */
    UInt16 PeAddr  :  3; /*!< B03..B05 = PE adddress, access via f.PeAddr               */
    UInt16 PixAddr : 10; /*!< B06..B15 = Pixel address = row, access via f.PixAddr      */
  }	F;
...
typedef struct {
  ...
  UInt32 TotRecSz; /*!<  Total record size, for info */
  SInt32 MaxFrNb;  /*!<  Max nb of frame allowed due to record size */
  SInt32 FrNb;     /*!<  Frames nb in source Acq converted in MIS1__TBtAcqW16A */
  
  UInt32 MSisW16Nb; /*!<  Size of each MSis 1 raw data stream */
  
  MIS1__TBtAcqRawHead AcqRawHead; /*!<  Copy of MIS1__TBtAcqRawRec Header field, may be remove later if useless */
  
  // UInt16 AMsis[MIS1__BT_MAX_MSIS_NB_ACQ][MIS1__BT_VRS_MAX_ACQ_DATA_SZ_W16]; 
  
  UInt16 AAMsis[MIS1__BT_MAX_MSIS_NB_ACQ][MIS1__BT_VRS_MAX_FR_NB_PER_ACQ * MIS1__BT_TOT_FRAME_SZ_W16]; 
...
typedef struct {
  ...
  UInt8  MSisId;                   /*!< MSis Id */
  UInt16 FrId;                     /*!<  Frame no - MAX 65535 !!! But max FlexRIO fr / Acq  = 65535  */
  
  MIS1__TDsFrHeader MSisFrHead;    /*!<  Header of MSis 1, as it is in data stream  */
  MIS1__TDsFrTrailer MSisFrTrail;  /*!<  Trailer of MSis 1, as it is in data stream */
  ...
  SInt32 ATrigPosW16[MIS1__BT_FR_DEC_MAX_TRIG_NB];  /*!< Position of triggers in this frame (up to MIS1__BT_FR_DEC_MAX_TRIG_NB) in W16  , < 0 => No trigger, not calculated / error*/

  UInt32 RegionNb;         /*!< Nb of regions   */
  UInt32 FiredPixNb;       /*!< Nb of fired pixels   */
...
/**
===================================================================================
* \struct MIS1__TBtAcqDec
*
* \brief Msis BT, decoded acquisiton, fixed size because : no time to develop dynamic, more efficient on exec time 
*
*
* G.CLAUS 24/05/2021
*
...
typedef struct {
  ...
  /*!< Detected Pixel nb in one frame => ResAAFrHead[MSisId][FrId].FiredPixNb */
  
  
  MIS1__TPixXY ResAAAFrPix[MIS1__BT_MAX_REAL_MSIS_NB_ACQ][MIS1__BT_VRS_MAX_FR_NB_PER_ACQ][MIS1__BT_FR_DEC_MAX_PIX_NB]; /*!< List of pixels in frame, indexes [MSisId][FrameId][PixelId]   */
