...
char* MIS1__FFrHeader2Str ( MIS1__TDsFrHeader* PtSrc, char* DestStr, SInt32 DestStrSz, SInt32 Mode ) {
  ...
  if ( Mode == 0 ) {
    ...
    sprintf ( VRetStr, "#H : [W7]=%X  [W6]=%X  [W5]=%X  [W4]=%X  [W3]=%X  [W2]=%X  [W1]=%X  [W0]=%x(H)  -  FrCnt = %d (D))", PtSrc->AW16[7], PtSrc->AW16[6], PtSrc->AW16[5], PtSrc->AW16[4], PtSrc->AW16[3], PtSrc->AW16[2], PtSrc->AW16[1], PtSrc->AW16[0], VFrCnt );
  }
...
/**
===================================================================================
* \fn      : MIS1__TBtAcqW16A* MIS1__BT_FBtAcqW16AAlloc ( UInt8 Alloc, UInt32* PtRecSz )
*          :
* \brief   : Allocates the MIS1__TBtAcqW16A records \n
*          : \n
*          :
* \param   : Alloc - 0 => Returns record size via PtRecSz, 1 => Alloc record + returns size
*          :
* \param   : PtRecSz - Pointer to record size, set to NULL if not used
*          :
* \return  : A pointer to IS1__TBtAcqW16A or NULL in case of error
*          :
* \warning : Globals   :
* \warning : Remark    : 
* \warning : Level     :
...
MIS1__TBtAcqW16A* MIS1__BT_FBtAcqW16AAlloc ( UInt8 Alloc, UInt32* PtRecSz ) {
  ...
  MIS1__TBtAcqW16A* VPtRec;
  UInt32            VRecSz;
  
  
  VRecSz = sizeof (MIS1__TBtAcqW16A);
  ...
  // Returns only size => No record allocation
  
  if ( Alloc == 0 ) {
    err_retval ( NULL, ( ERR_OUT, "Ok, retunrs size only, no record allocation") );
  }
  

  VPtRec = (MIS1__TBtAcqW16A*) malloc ( VRecSz );
  
  if ( VPtRec == NULL ) {
    err_error (( ERR_OUT, "Abort => Allocation of %d W8 fro MIS1__TBtAcqW16A failed ! ", VRecSz  ));
    return (NULL);
  }
  ...
  memset ( VPtRec, 0, VRecSz );

  // Set info fields

  VPtRec->TotRecSz = VRecSz;
  VPtRec->MaxFrNb  = MIS1__BT_VRS_MAX_FR_NB_PER_ACQ;
  VPtRec->FrNb     = 0;

  err_trace (( ERR_OUT, "Ok" ));
...
/**
===================================================================================
* \fn      : SInt32 MIS1__BT_FBtAcqW16AFree ( MIS1__TBtAcqW16A* PtRec )
*          :
* \brief   : Free the MIS1__TBtAcqW16A* record  \n
*          : \n
*          :
* \param   : PtRec - The record to free
*          :
*          :
* \return  : Error code
*          :   0 - OK
...
/**
===================================================================================
* \fn      : double MIS1__BT_FBtAcqW16AFill ( MIS1__TBtAcqRawRec* PtSrc, MIS1__TBtAcqW16A* PtDest, SInt32 FrNb, UInt8 MeasExecTime, UInt8 PrintLvl, UInt8 FuncVers )
*          :
* \brief   : Convert MIS1__TBtAcqRawRec to MIS1__TBtAcqW16A record, FuncVers sets the function used to convert  \n
*          : \n
*          :
...
*          :
* \param   : FuncVers     - Select the conversion function used => in order to test different implementations \n 
*          :              0 => MIS1__BT_FBtAcqW16AFill_v0, 1 => MIS1__BT_FBtAcqW16AFill_v1, etc
*          :
...
double MIS1__BT_FBtAcqW16AFill ( MIS1__TBtAcqRawRec* PtSrc, MIS1__TBtAcqW16A* PtDest, SInt32 FrNb, UInt8 MeasExecTime, UInt8 PrintLvl, UInt8 FuncVers ) {
  ...
  if ( FrNb == 0 ) {
    err_retfail ( -1, (ERR_OUT,"Abort => FrNb == 0 => Nothing to do") );
  }
  
  if ( FrNb > MIS1__BT_FRS_MAX_FR_NB_PER_ACQ ) {
    err_retfail ( -1, (ERR_OUT,"Abort => FrNb = %d > MIS1__BT_FRS_MAX_FR_NB_PER_ACQ = %d ", FrNb, MIS1__BT_FRS_MAX_FR_NB_PER_ACQ ) );
  }
  
  if ( FrNb > PtSrc->Head.Res.FrNb ) {
    err_retfail ( -1, (ERR_OUT,"Abort => FrNb = %d > PtSrc->Head.Res.FrNb = %d ", FrNb, PtSrc->Head.Res.FrNb ) );
  }
  
  
  if ( FrNb > PtDest->MaxFrNb ) {
    err_retfail ( -1, (ERR_OUT,"Abort => FrNb = %d > PtDest->MaxFrNb = %d ", FrNb, PtDest->MaxFrNb ) );
  }
  
  // Propagates AcqId info to destination MIS1__TBtAcqW16A
  ...
  switch ( FuncVers ) {
    ...
    case 0 : {
      VRet = MIS1__BT_FBtAcqW16AFill_v0 ( PtSrc, PtDest, FrNb, MeasExecTime, PrintLvl );
      break; }
  

    case 10 : {
      VRet = MIS1__BT_FBtAcqW16AFill_v10 ( PtSrc, PtDest, FrNb, MeasExecTime, PrintLvl );
      break; }
    
    
    case 80 : {
      VRet = MIS1__BT_FBtAcqW16AFill_v80 ( PtSrc, PtDest, FrNb, MeasExecTime, PrintLvl );
      break; }
  
  
    default : {
      err_retfail ( -1, (ERR_OUT,"Abort => FuncVers = %d which is not supported", FuncVers) );
      break; }
  ...
  err_retfail ( VRet, (ERR_OUT,"Abort => Conversion failed ! Conv function ret = %d", VRet ) );
...
/**
===================================================================================
* \fn      : double MIS1__BT_FBtAcqW16AFill_v0 ( MIS1__TBtAcqRawRec* PtSrc, MIS1__TBtAcqW16A* PtDest, SInt32 FrNb, UInt8 MeasExecTime, UInt8 PrintLvl )
*          :
* \brief   : Convert MIS1__TBtAcqRawRec to MIS1__TBtAcqW16A record, SIMPLE COPY, implementation V0  \n
*          :
* \param   : PtSrc    - Pointer to source record MIS1__TBtAcqRawRec
*          :
* \param   : PtDest   - Pointer to destination record MIS1__TBtAcqW16A
*          :
* \param   : FrNb     - Frames nb to convert, if -1 => All frames from  source record, value <>  -1 are not handled
*          :
* \param   : MeasExecTime - Measure exec time 0 = No, 1 = Yes
*
* \param   : PrintLvl     - Debug print level, 0 = No, 1 = Print record sizes, 2 = More print, to be implemented
...
*          :
* \warning : Globals   :
* \warning : Remark    : WARNING THis function makes a simple copy, no conversion, used to measure execution time of memcpy
* \warning : Level     :
*          :
* \warning : Items not filled now :
*  todo    :
...
double MIS1__BT_FBtAcqW16AFill_v0 ( MIS1__TBtAcqRawRec* PtSrc, MIS1__TBtAcqW16A* PtDest, SInt32 FrNb, UInt8 MeasExecTime, UInt8 PrintLvl ) {
  ...
  PtDest->MSisW16Nb = 0; // Size of each MSis 1 "cnanel" in W16 => Set to 0 to generate an error
                         // because data are not formatted by this funciton for pxiels decoding in a next step
  
  // Convert => Simple cpy
    
  memcpy ( &PtDest->AAMsis[0][0], PtSrc->MSisData.Au8, PtSrc->Head.DataSz );
...
/**
===================================================================================
* \fn      : double MIS1__BT_FBtAcqW16AFill_v10 ( MIS1__TBtAcqRawRec* PtSrc, MIS1__TBtAcqW16A* PtDest, SInt32 FrNb, UInt8 MeasExecTime, UInt8 PrintLvl )
*          :
* \brief   : Convert MIS1__TBtAcqRawRec to MIS1__TBtAcqW16A record, for 1 channel, implementation V10  \n
*          :
* \param   : PtSrc    - Pointer to source record MIS1__TBtAcqRawRec
*          :
* \param   : PtDest   - Pointer to destination record MIS1__TBtAcqW16A
...
double MIS1__BT_FBtAcqW16AFill_v10 ( MIS1__TBtAcqRawRec* PtSrc, MIS1__TBtAcqW16A* PtDest, SInt32 FrNb, UInt8 MeasExecTime, UInt8 PrintLvl ) {
  ...
  UInt64* VPtSLowW64;     // Pt on src low half of acq memory
  UInt64* VPtSHighW64;    // Pt on src low half of acq memory
  ...
  UInt32 ViW16;
  SInt32 VNotAnIntegerW128NbPerFr;
  ...
  VPtSLowW64     = PtSrc->MSisData.Au64;
  VPtSHighW64    = &PtSrc->MSisData.Au64[VSrcLowOfsW64];
  
  // Chan 0 ptr init

  VPtSCh0Low   = (UInt16*) VPtSLowW64;
  VPtSCh0High  = (UInt16*) VPtSHighW64;
  ...
  for ( ViW128 = 0; ViW128 < VSrcChanSzW128; ViW128++ ) {
    ...
    VPtDCh0[ViW16] = *VPtSCh0Low; // W0
    VPtSCh0Low += 8;
    ViW16++;
    VPtDCh0[ViW16] = *VPtSCh0Low; // W1
    VPtSCh0Low += 8;
    ViW16++;
    VPtDCh0[ViW16] = *VPtSCh0Low; // W2
    VPtSCh0Low += 8;
    ViW16++;
    VPtDCh0[ViW16] = *VPtSCh0Low; // W3
    VPtSCh0Low += 8;
    ViW16++;
  

    VPtDCh0[ViW16] = *VPtSCh0High; // W4
    VPtSCh0High += 8;
    ViW16++;
    VPtDCh0[ViW16] = *VPtSCh0High; // W5
    VPtSCh0High += 8;
    ViW16++;
    VPtDCh0[ViW16] = *VPtSCh0High; // W6
    VPtSCh0High += 8;
    ViW16++;
    VPtDCh0[ViW16] = *VPtSCh0High; // W7
    VPtSCh0High += 8;
    ViW16++;
...
/**
===================================================================================
* \fn      : SInt32 MIS1__BT_FBtAcqW16APrintMSisU16 ( MIS1__TBtAcqW16A* Pt, UInt8 MSisId, UInt32 FirstW16, UInt32 W16Nb )
*          :
* \brief   : Print the raw data of one MSis of MIS1__TBtAcqW16A* record  \n
*          :
* \param   : Pt        - Pointer to record MIS1__TBtAcqRawRec
*          :
* \param   : MSisId    - 0..5 = MSis 0 to 5, 6,7 = Trigegrs channels
*          :
* \param   : FirstW16  - First W16 to print
*          :
* \param   : W16Nb     - Nb of W16 to print
*          :
*          :
* \return  : Execution time in us or error code
*          : >=  0 - Execution time in us
...
SInt32 MIS1__BT_FBtAcqW16APrintMSisU16 ( MIS1__TBtAcqW16A* Pt, UInt8 MSisId, UInt32 FirstW16, UInt32 W16Nb ) {
  ...
  SInt32 VRet;
  SInt32 VLastW16;
  SInt32 VMaxW16NbPerChan; // Calculated from record size
  SInt32 VW16NbPerChan;    // Calculated from number of frames in record
  SInt32 ViW16;
  ...
  if ( MSisId >= MIS1__BT_MAX_MSIS_NB_ACQ ) {
    err_retfail ( -1, (ERR_OUT,"Abort => MSisId = %d >= MIS1__BT_MAX_MSIS_NB_ACQ = %d", MSisId, MIS1__BT_MAX_MSIS_NB_ACQ) );
  }
  ...
  if ( W16Nb == 0 ) {
    ...
    VLastW16 = FirstW16 + W16Nb - 1;
  }
  
  VMaxW16NbPerChan = MIS1__BT_VRS_MAX_ACQ_DATA_SZ_W16;
  VW16NbPerChan    = MIS1__BT_TOT_FRAME_SZ_W16 * Pt->FrNb;
  
  if (VLastW16 >= VMaxW16NbPerChan) {
    err_retfail ( -1, (ERR_OUT,"Abort => VLastW16 = %d >= Max W16 nb / chan in record = %d", VLastW16, VMaxW16NbPerChan) );
  }
  
  if (VLastW16 >= VW16NbPerChan) {
    err_retfail ( -1, (ERR_OUT,"Abort => VLastW16 = %d >= W16 nb in record = %d (function of frames nb)", VLastW16, VW16NbPerChan) );
  }
  ...
  msg (( MSG_OUT, "" ));
  msg (( MSG_OUT, "Print MSis No %d, W16 from no %d to %d (%d W16 total)", MSisId, FirstW16, VLastW16, W16Nb  ));
  msg (( MSG_OUT, "" ));
  
  for ( ViW16 = FirstW16; ViW16 <= VLastW16; ViW16++ ) {
    
    msg (( MSG_OUT, "MSis[%d] W16[%.4d] = %X", MSisId, ViW16, Pt->AAMsis[MSisId][ViW16] ));
...
/**
===================================================================================
* \fn      : MIS1__TBtAcqDec* MIS1__BT_FBtAcqDecAlloc ( UInt8 Alloc, UInt32* PtRecSz )
*          :
* \brief   : Allocates the MIS1__TBtAcqDec record \n
*          : \n
*          :
* \param   : Alloc - 0 => Returns record size via PtRecSz, 1 => Alloc record + returns size
*          :
* \param   : PtRecSz - Pointer to record size, set to NULL if not used
*          :
* \return  : A pointer to MIS1__TBtAcqDec or NULL in case of error
*          :
* \warning : Globals   :
* \warning : Remark    :
* \warning : Level     :
...
MIS1__TBtAcqDec* MIS1__BT_FBtAcqDecAlloc ( UInt8 Alloc, UInt32* PtRecSz ) {
  ...
  MIS1__TBtAcqDec*  VPtRec;
  UInt32            VRecSz;
  
  
  VRecSz = sizeof (MIS1__TBtAcqDec);
  ...
  VPtRec = (MIS1__TBtAcqDec*) malloc ( VRecSz );
  
  if ( VPtRec == NULL ) {
    err_error (( ERR_OUT, "Abort => Allocation of %d W8 fro MIS1__TBtAcqDec failed ! ", VRecSz  ));
    return (NULL);
  }
...
/**
===================================================================================
* \fn      : SInt32 MIS1__BT_FBtAcqDecFree ( MIS1__TBtAcqDec* PtRec )
*          :
* \brief   : Free the  MIS1__TBtAcqDec* record  \n
*          : \n
*          :
* \param   : PtRec - The record to free
*          :
...
/**
===================================================================================
* \fn      : SInt32 MIS1__BT_FFrDecHeadPrint ( MIS1__TBtFrDecHead* Pt, UInt8 PrintMode )
*          :
* \brief   : Pritns frame header \n
*          : \n
*          :
* \param   : Pt - Pointer to header
*          :
* \param   : PrintMode -  0 => Print all, 1 => Print only region, fired pixels cnt
*          :
* \return  : Error code
*          :   0 - OK
*          : < 0 - Error
...
SInt32 MIS1__BT_FFrDecHeadPrint ( MIS1__TBtFrDecHead* Pt, UInt8 PrintMode ) {
  ...
  char VStrHead[GLB_CMT_SZ];
  char VStrTrail[GLB_CMT_SZ];

  // Check param

  err_retnull ( Pt, (ERR_OUT,"Abort Pt == NULL") );

  // Convert head, trail to sting

  MIS1__FFrHeader2Str  ( &Pt->MSisFrHead, VStrHead, GLB_CMT_SZ, 1 /* Mode */ );
  MIS1__FFrTrailer2Str ( &Pt->MSisFrTrail, VStrTrail, GLB_CMT_SZ, 0 /* Mode */ );
  ...
  if ( PrintMode == 0 ) {
    msg (( MSG_OUT, "" ));
    msg (( MSG_OUT, "------------------------------------------------------" ));
    msg (( MSG_OUT, "Fr header MSis = %d, FrId = %.4d ", Pt->MSisId, Pt->FrId ));
    msg (( MSG_OUT, "------------------------------------------------------" ));
    msg (( MSG_OUT, "MSis FrCnt      = %.4d", Pt->FrCnt ));
    msg (( MSG_OUT, "CheckSum        = %.4d", Pt->CheckSum ));
    ...
    msg (( MSG_OUT, "ATrigPosW16[1]  = %.4d", Pt->ATrigPosW16[1] ));
    msg (( MSG_OUT, "------------------------------------------------------" ));
    msg (( MSG_OUT, "" ));
    err_retok (( ERR_OUT, "Ok" ));
  } // End if ( Mode == 0 )
...
/**
===================================================================================
* \fn      : SInt32 MIS1__BT_FAcqDecPrintGen ( MIS1__TBtAcqDec* Pt, SInt8 MSisId, UInt8 PrintMode )
*          :
* \brief   : Prints acq dec general fields, for ResAAFrHead => MIS1__BT_FFrDecHeadPrint (...) \n
*          : \n
*          :
* \param   : Pt - Pointer to AcqDec
*          :
* \param   : MSisId - Id of MSis 1 to print, -1 => ALL
*          :
...
SInt32 MIS1__BT_FAcqDecPrintGen ( MIS1__TBtAcqDec* Pt, SInt8 MSisId, UInt8 PrintMode ) {
  ...
  msg (( MSG_OUT, "------------------------------------------------------" ));
  msg (( MSG_OUT, "" ));
  msg (( MSG_OUT, "ParFrNbInSrcAcq = %d", Pt->ParFrNbInSrcAcq ));
  
  if ( MSisId >= 0 ) {
    msg (( MSG_OUT, "" ));
    msg (( MSG_OUT, "MSisId = %d", MSisId ));
    msg (( MSG_OUT, "" ));
    msg (( MSG_OUT, "ResAFrNb          = %.3d", Pt->ResAFrNb[MSisId] ));
    msg (( MSG_OUT, "ResAFrFakeHeadCnt = %.3d", Pt->ResAFrFakeHeadCnt[MSisId] ));
    msg (( MSG_OUT, "ResAFrNbWoTrailer = %.3d", Pt->ResAFrNbWoTrailer[MSisId] ));
    msg (( MSG_OUT, "RegionNb          = %.3d", Pt->ResARegNb[MSisId] ));
    msg (( MSG_OUT, "FiredPixNb        = %.3d", Pt->ResAPixNb[MSisId] ));
    msg (( MSG_OUT, "" ));
    err_retok (( ERR_OUT, "Ok" ));
  }
  ...
  if ( MSisId  < 0 ) {
    ...
    for ( ViMsis = 0; ViMsis < MIS1__BT_MAX_REAL_MSIS_NB_ACQ; ViMsis++ ) {
      ...
      msg (( MSG_OUT, "MSisId = %d", MSisId ));
      msg (( MSG_OUT, "" ));
      msg (( MSG_OUT, "ResAFrNb          = %.3d", Pt->ResAFrNb[ViMsis] ));
      msg (( MSG_OUT, "ResAFrFakeHeadCnt = %.3d", Pt->ResAFrFakeHeadCnt[ViMsis] ));
      msg (( MSG_OUT, "ResAFrNbWoTrailer = %.3d", Pt->ResAFrNbWoTrailer[ViMsis] ));
      msg (( MSG_OUT, "RegionNb          = %.3d", Pt->ResARegNb[ViMsis] ));
      msg (( MSG_OUT, "FiredPixNb        = %.3d", Pt->ResAPixNb[ViMsis] ));
      msg (( MSG_OUT, "" ));
    } // End for
...
/**
===================================================================================
* \fn      : SInt32 MIS1__BT_FAcqDecPrintPix ( MIS1__TBtAcqDec* Pt, SInt8 MSisId, UInt32 FrId, UInt32 MaxPixToPrint, UInt8 PrintMode )
*          :
* \brief   : Prints acq dec pixels \n
*          : \n
*          
* \param   : Pt            - Pointer to AcqDec
*          
...
*          
* \param   : FrId          - Id of the frame to print
*
* \param   : MaxPixToPrint - Maximum number of pixels to print, to avoid "endless" printing loop ...
*
* \param   : PrintMode     - For future use
*
* \return  : Error code
*          :   0 - OK
*          : < 0 - Error ( -1 sw error, -2 more than MaxPixToPrint => exits before end 
*          :
* \warning : Globals   :
* \warning : Remark    :
* \warning : Level     :
...
SInt32 MIS1__BT_FAcqDecPrintPix ( MIS1__TBtAcqDec* Pt, SInt8 MSisId, UInt32 FrId, UInt32 MaxPixToPrint, UInt8 PrintMode ) {
  ...
  msg (( MSG_OUT, "" ));
  msg (( MSG_OUT, "------------------------------------------------------" ));
  msg (( MSG_OUT, "Pixels acq dec : AcqId = %.4d, MSisId = %d, FrId = %.4d",  Pt->AcqId, MSisId, FrId ));
  msg (( MSG_OUT, "------------------------------------------------------" ));
  msg (( MSG_OUT, "" ));
  
  for ( ViPix = 0; ViPix < Pt->ResAAFrHead[MSisId][FrId].FiredPixNb; ViPix++ ) {
    
    if ( ViPix >= MaxPixToPrint ) {
      msg (( MSG_OUT, "" ));
      msg (( MSG_OUT, "Stop before end : Max pixels nb to print = %d reached", MaxPixToPrint ));
      err_retfail ( -2, (ERR_OUT,"%s", MSG_OUT) );
    }
    
    VPix = Pt->ResAAAFrPix[MSisId][FrId][ViPix];
    
    msg (( MSG_OUT, "Pixel[%.4d]:  Y = %.4d - X  = %.4d", ViPix, VPix.C.y, VPix.C.x ));
